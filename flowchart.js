          ...
          if (line.indexOf("=>") >= 0) {
            ...
            for (
              var lineStyleSymbols = line.split("@>"),
                i = 0,
                lenS = lineStyleSymbols.length;
              i < lenS;
              i++
            )
              if (i + 1 != lenS) {
                var curSymb = getSymbol(lineStyleSymbols[i]),
                  nextSymb = getSymbol(lineStyleSymbols[i + 1]);
                curSymb.lineStyle[nextSymb.key] = JSON.parse(
                  getStyle(lineStyleSymbols[i + 1])
                );
              }
        }
        return chart;
